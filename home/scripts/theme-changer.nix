...
pkgs.writeShellScriptBin "theme-changer" ''
  ...
  file_map[fastfetch]="fastfetch_img.png"
  file_map[rofi-img]="rofi.png"
  file_map[rofi]="rofi.rasi"
  file_map[kitty]="kitty.conf"
  file_map[waybar]="waybar.css"
  file_map[hypr]="hypr.lua"

  all_components=(wallpaper lockpaper fastfetch rofi-img rofi kitty waybar hypr)
  ...
  get_current_theme() {
    ...
    source=$(readlink -f "$CURRENT/wallpaper.png")
    theme_dir=$(dirname "$source")
  ...
  show_usage() {
    ...
    echo "  theme-changer wallpaper marathon"
    exit 1
  }
  ...
  case "''${1:-}" in
    ...
    themes)
      ...
      list_themes | sed 's/^/ /'
      ;;
    wallpaper|lockpaper|fastfetch|rofi-img|rofi|kitty|waybar|hypr)
      if [[ -z "''${2:-}" ]]; then
        show_usage
      fi
      set_single "$1" "$2"
