...

{
  wayland.windowManager.sway = {
    ...
    in {
      ...
      startup = [
        { command = "theme-changer init"; }
        { command = "awww-daemon --no-cache"; }
        { command = "waybar"; }
        { command = "awww img ~/.config/rice/current/wallpaper.png"; }
      ];
      ...
      window = {
        border = 2;
        commands = [
          { criteria = { title = ".*pdf.*"; }; command = "opacity 0.85"; }
          { criteria = { class = "firefox"; }; command = "opacity 1.0"; }
          { criteria = { class = "Spotify"; }; command = "opacity 0.78"; }
          { criteria = { title = ".*Vesktop.*"; }; command = "opacity 0.88"; }
      ...
      output = if hostName == "Ghylak" then {
        "HDMI-A-1" = { mode = "1920x1080@60Hz"; pos = "0 180"; scale = "1"; };
        "DP-3" = { mode = "2560x1440@60Hz"; pos = "1920 0"; scale = "1.33"; };
      } else if hostName == "Murgo" then {
        "eDP-1" = { mode = "1920x1080@60Hz"; pos = "0 0"; scale = "1"; };
      } else {
        "*" = { bg = "#000000 solid_color"; };
      };

      workspaceOutputAssign = if hostName == "Ghylak" then [
